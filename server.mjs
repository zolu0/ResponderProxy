...
async function xRequest(path) {
  ...
  if (!response.ok)
    throw new Error(`X API ${response.status}: ${JSON.stringify(body)}`);
  return body;
}
  ...
  try {
    ...
    const accountPosts = await Promise.all(
      xUsernames.map(async (username) => {
        ...
        if (
          typeof sinceIds[username] === 'string' &&
          /^\d+$/.test(sinceIds[username])
        ) {
  ...
  try {
    ...
    const apiResponse = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${encodeURIComponent(apiKey)}`,
      {
        ...
        body: JSON.stringify({
          contents: [{ parts: [{ text: request.body.prompt }] }],
        }),
      },
    );
    const data = await apiResponse.json();
    if (!apiResponse.ok)
      throw new Error(`Gemini ${apiResponse.status}: ${JSON.stringify(data)}`);
    response.json({
